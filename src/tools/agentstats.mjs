...
const val = (name) => (args.indexOf(name) === -1 ? null : args[args.indexOf(name) + 1]);

const DURATION = { s: 1e3, m: 6e4, h: 36e5, d: 864e5 };
const parseDur = (s) => {
  const m = /^(\d+)([smhd])$/.exec(s || '');
  return m ? Number(m[1]) * DURATION[m[2]] : null;
};
...
async function scan(file, since) {
  const st = { turns: 0, first: 0, last: 0, ctx: 0, imgBytes: 0, waits: [], lastTool: null, pending: null };
  let resultAt = 0;
...
const pct = (v, p) => (v.length ? v.slice().sort((a, b) => a - b)[Math.min(v.length - 1, Math.floor(v.length * p))] : 0);
const fmtSec = (s) => (s ? `${Math.round(s)}s` : '-');
