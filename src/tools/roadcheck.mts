...
// ---------------------------------------------------------------- 2. grades
console.log('\n2. grades');
let worstGrade: { g: number, e?: string, lim?: number } = { g: 0 };
const gradeFails = [];
...
console.log(`  worst grade ${(worstGrade.g * 100).toFixed(1)}% on ${worstGrade.e} (limit ${((worstGrade.lim ?? 0) * 100).toFixed(0)}%)`);

// -------------------------------------------------------------- 3. corners
console.log('\n3. corner radii');
let tightest: { r: number, e?: string, lim?: number } = { r: Infinity };
const cornerFails = [];
for (const e of g.edges) {
  const rr = g.radii(e);
