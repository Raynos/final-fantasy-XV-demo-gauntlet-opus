 ...
 * invented numbers on top of it. `tools/orphans.mjs` catches modules nobody
 ...
 *   node tools/integration.mjs --json
 */
...
const results = await page.evaluate(async () => {
  ...
  const add = (area, name, status, evidence) => out.push({ area, name, status, evidence });
  const probe = (area, name, fn) => {
    try {
      const r = fn();
      if (!r) return add(area, name, 'FAIL', 'probe returned nothing');
      add(area, name, r.status, r.evidence);
    } catch (e) { add(area, name, 'FAIL', 'threw: ' + (e && e.message)); }
  };
  const P = (evidence) => ({ status: 'PASS', evidence });
  const W = (evidence) => ({ status: 'WIRED', evidence });
  ...
  probe('rpg', 'ascension spends real AP', () => {
    ...
    const cand = (a.availableNodes ? a.availableNodes() : []).find((n) => (n.cost ?? 0) <= ap);
...
for (const r of results) {
  ...
  console.log(`${mark[r.status]}  ${r.name.padEnd(44)} ${r.evidence}`);
}
