...
await page.evaluate(() => {
  ...
  window.key = (code, type = 'keydown') => window.dispatchEvent(
    new KeyboardEvent(type, { code, bubbles: true }));
  window.press = (code, frames = 3) => { window.key(code); window.step(1); window.key(code, 'keyup'); window.step(frames); };
});
...
const lock = await page.evaluate(async () => {
  ...
  const row = document.querySelector('#menus .mrow');
...
const live = await page.evaluate(async () => {
  ...
  const wanted = gear.picker.rows.find((r) => r.id);
  if (wanted) {
