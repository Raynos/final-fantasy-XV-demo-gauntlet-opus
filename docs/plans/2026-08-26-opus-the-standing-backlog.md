...
standing out as clean black silhouettes. Canopy `d1` **9.17 → 8.45** against
the reference plate's 8.58, `d8` and above unchanged to two decimals; treeline
`edge%` 42.71 → 39.39 and p90 60.7 → 57.9; near-crown speckle 36.4 → 26.5. All
three states `--cold` off one committed sha. `perf` 0 of 5 shots over the
floor, `gameplay` PASS, `check` 19/19, `nanscan` 0 of 142.
...
| PCSS blocker search on our shadow path | needs a depth read `sampler2DShadow` cannot do. **Its second clause — "and the page is already at 16/16 texture units" — is void**, see the row above: the page is at 17 of a combined 32 and three was counting the wrong limit. Still closed, on one clause instead of two |
...
| **`probes/stackjoint.mts` says 0 open joints of 1615, so the stacked-rock joints are closed** | **it is measuring the recipe against itself.** `hullExtents` is a `computeBoundingBox` — `ext[1]` is a block's greatest half-height *anywhere* — and `stackPlan`, `torPlan` and `_genOutcrop` all author their overlap through it, while the contact between two courses is at the **middle of the face**, where a warped icosahedron's surface sits above its own bbox minimum. `stackjoint` composes the shipped plan through the shipped `placedScale`, deliberately, and `placedScale` reads the same `ext`. `probes/hullseat.mts` reads the **mesh**: shortfall as a fraction of the half-height is `slab` **0.139**, `pebble` 0.120, `spire` 0.081, `cobble` 0.080, `bedded` 0.052, and granite/worn/talus 0.000. A joint pays both blocks, so a slab-on-slab course at a 4.4 m long axis opens about **0.55 m** — which is the gap in `tmp/shots/lr2-impp/rock.png`, a three-course tor in `poi_imperial` with daylight all the way across its top joint, unchanged by every joint fix so far. `Seat.supportPoints` already states the rule this breaks: *enforce guarantees on the finished, placed mesh, not on the recipe*. **The fix is a second extents map — the surface height under the axis — used for the joint arithmetic while `ext` keeps doing widths; price it against `silhouette --set rocks` first, because the last unclamped joint fix breached two of its floors** |
...
| **WS-3.2: a depth mask can take CAS's foliage cost down to what `?post=nocas` shows** | **about a third of it, and the rest is not a depth phenomenon.** The mask lands treeline `edge%` 42.71 -> 39.39 where `nocas` is 28.26, and near-crown speckle 36.4 -> 26.5 where `nocas` is 7.1. What survives is CAS sharpening *within* a card: a distant tree is three impostor planes and a mid tree is crossed cards, so the leaf detail is in the alpha and albedo texture and the depth buffer sees one flat surface across it. A depth mask structurally cannot see texture aliasing. The part it does reach — the silhouette and the multi-card fringe — is the part the judge named. Landed at `95a34c0` |
