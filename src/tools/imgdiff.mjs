...
/** @returns {{w:number, h:number, data:Uint8Array}} RGBA8 */
...
/** @returns {{mean:number, max:number, over:number}} per-channel deltas in 0..255 */
...
async function main() {
  ...
  if (!aPath || !bPath) { console.error('usage: imgdiff.mjs <a> <b> [--max 2]'); process.exit(2); }
  let limit = 2;
  ...
  for (const name of names) {
    ...
    catch (e) { console.log(`${(name || '').padEnd(20)} ERROR ${e.message}`); bad++; continue; }
