...
export function corrodedSteel(tint = 0x4a4038) {
  ...
  const h = (u: number, v: number) => (n.fbm2(u * 22, v * 22, 4) * 0.5 + 0.5) * 0.55
  ...
  return pbr(`corrodedSteel${tint}`, {
    ...
    metalMap: (u: number, v: number) => 1 - ss(0.30, 0.72, n.fbm2(u * 5.5, v * 5.5, 4) * 0.5 + 0.5) * 0.9,
  });
}
...
export function oreSeam() {
  ...
  return pbr('oreSeam', {
    ...
    metalMap: (u: number, v: number) => 0.15 + (1 - ss(0.0, 0.16, Math.abs(n.fbm2(u * 5, v * 12, 4)))) * 0.85,
  });
}
...
export function railSteel() {
  ...
  const h = (u: number, v: number) => (n.fbm2(u * 30, v * 8, 3) * 0.5 + 0.5) * 0.6 + n.worley2(u * 20, v * 6).f1 * 0.4;
