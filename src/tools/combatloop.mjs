...
const results = await page.evaluate(async () => {
  const g = window.GAME;
  const out = [];
  const add = (name, ok, evidence) => out.push({ name, ok: !!ok, evidence: String(evidence) });
  const check = (name, fn) => {
    try {
      const r = fn();
      add(name, r && r.ok, r ? r.evidence : 'no result');
    } catch (e) { add(name, false, 'threw: ' + (e && e.message)); }
  };
  const P = (evidence) => ({ ok: true, evidence });
  const F = (evidence) => ({ ok: false, evidence });
  ...
  const hold = (code, frames) => { keyDown(code); step(frames); keyUp(code); step(1); };
  const mouseDown = (button) => window.dispatchEvent(new MouseEvent('mousedown', { button, bubbles: true }));
  const mouseUp = (button) => window.dispatchEvent(new MouseEvent('mouseup', { button, bubbles: true }));
  const holdMouse = (button, frames) => { mouseDown(button); step(frames); mouseUp(button); step(1); };
  ...
  const spawnAhead = (key, d = 1.6, opts = {}) => {
  ...
  const pin = (e) => { e.frozenPose = { state: 'idle', phase: 0 }; return e; };

  /** Make the pack notice you, so the encounter state — and the combat HUD
   *  layer that hangs off it — is actually up. */
  const engage = (e) => { e.target = player; e.awareness = 1; e.setState('chase'); return e; };
  ...
  check('damage number == Stats.computeDamage output', () => {
    ...
    combat.drawSlot(0); step(2);
    const e = pin(spawnAhead('sabertusk'));
    let seen = null; let roll = null;
    const off = combat.on('damage', (d) => {
  ...
  check('companion techniques fire on G / J / K', () => {
    ...
    combat.armigerTimer = 0;
    const fired = [];
    const onTech = (ev) => fired.push(ev.detail.member + ':' + ev.detail.tech);
  ...
  check('craft a spell and equip it', () => {
    ...
    const mix = {}; mix[dep.element] = Math.min(have, 30);
  ...
  check('enemy nameplates track live HP', () => {
    ...
    const plates = [...document.querySelectorAll('.nameplate')].filter((n) => n.style.display !== 'none');
    if (!plates.length) return F('no nameplate in the DOM');
    const shown = plates.map((n) => `${n.querySelector('.np-name').textContent}@${n.querySelector('.gauge i.fill').style.width}`);
    const live = enemies.list.map((x) => `${x.name}:${(x.hp / x.maxHp * 100).toFixed(0)}%${x.dead ? '(dead)' : ''}`);
    const p0 = plates[0];
    const name = p0.querySelector('.np-name').textContent;
    const lv = p0.querySelector('.np-lv').textContent;
    const w = parseFloat(p0.querySelector('.gauge i.fill').style.width);
    const want = 40;
  ...
  check('damage numbers appear on the HUD', () => {
    ...
    let last = 0;
    const off = combat.on('damage', (d) => { if (!d.source) last = d.damage; });
    holdMouse(0, 90);
    step(2);
    off();
    const nodes = [...document.querySelectorAll('.dmg .dv')].map((n) => Number(n.textContent.replace(/,/g, '')));
  ...
  check('Armiger gauge on the HUD reads the earned value', () => {
    ...
    const pct = document.querySelector('.ar-pct');
  ...
  check('kill an enemy -> EXP, gil and a drop roll', () => {
    ...
    const kills = [];
    const onKill = (ev) => kills.push(ev.detail);
