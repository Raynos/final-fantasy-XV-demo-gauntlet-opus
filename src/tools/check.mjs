...
const GATES = [
  ...
  { name: 'orphans', script: 'orphans.mjs', expect: 'every module reachable' },
  { name: 'integration', script: 'integration.mjs', expect: '18 pass, 0 fail' },
  { name: 'uxcheck', script: 'uxcheck.mjs', expect: '86/86' },
  { name: 'creaturecheck', script: 'creaturecheck.mjs', expect: '207 poses, 0 failures' },
  { name: 'combatloop', script: 'combatloop.mjs', expect: '30/30' },
  { name: 'roadcheck', script: 'roadcheck.mjs', expect: '0 failures' },
  ...
  { name: 'heightcheck', script: 'heightcheck.mjs', expect: '0.000 m GPU vs CPU', needsServer: true },
  { name: 'driftcheck', script: 'driftcheck.mjs', expect: 'within tolerance', needsServer: true },
  { name: 'perf', script: 'perf.mjs', expect: '60 fps', perf: true },
  { name: 'gameplay', script: 'gameplay.mjs', expect: '60 fps under real input', perf: true },
];

function parse(argv) {
  const o = { perf: false, only: null };
...
function serve(port) {
  return new Promise((resolve, reject) => {
    ...
    let out = '';
    const done = (ok) => { if (ok) resolve(p); else reject(new Error(out.slice(-400))); };
