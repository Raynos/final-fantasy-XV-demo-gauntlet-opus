...
import { ENEMY_TEMPLATES, TECHNIQUES } from './GameData.js';
...
export class CombatHUD {
  ...
  _buildTechs() {
    const node = el('div.techs');
    const rows = TECHNIQUES.map((t, i) => {
      ...
      const row = el('div.tech', {}, [
        ...
        el('div.tk-body', {}, [
          ...
          el('div.tk-ow', { text: t.owner }),
          b.node,
        ]),
        el('div.tk-key', { text: ['L1+□', 'L1+△', 'L1+○'][i] || '' }),
      ]);
  ...
  damage(ev) {
    ...
    this.numbers.push({
      ...
      size: clamp(12.5 + Math.pow(amount, 0.34) * 1.55, 15, 27) * (crit ? 1.34 : 1),
      clip: new Clip(crit ? 1.35 : 1.05),
    });
    if (this.numbers.length > 40) this._retire(this.numbers.shift());
  }
  ...
  update(dt, game, appear) {
    ...
    if (!this._armigerDriven) {
      ...
      this.armigerVal = typeof live === 'number' ? clamp(live, 0, 1)
    ...
    this.techs.rows.forEach((r, i) => {
      ...
      const ready = typeof live?.ready === 'number' ? live.ready : r.t.ready;
      r.bar.set(ready, dt);
    ...
    if (Array.isArray(live) && live.length) {
      return live.slice(0, 5).map((e2, i) => {
        ...
        return {
          ...
          weak: e2.weak || tpl.weak,
          height: e2.height ?? 2.0,
          pos,
  ...
  /** Deterministic stand-in encounter: chip damage, hit numbers, call-outs. */
  _mock(dt, game, enemies) {
    if (!enemies.length || !enemies[0].mock) return;
    this.mockT += dt;
    // enemies drift slightly so nameplates visibly track world space
    ...
    while (this._beat < beats.length && beats[this._beat] <= this.mockT) {
      ...
      const crit = i % 4 === 2;
      const amount = crit ? 900 + Math.floor(r() * 700) : 120 + Math.floor(r() * 260);
      tgt.hp = Math.max(1, tgt.hp - amount * 0.35);
      this.damage({
    ...
    if (!this._didCall && this.mockT > 0.55) {
      this.callOut('Blindside!', 'Attack from behind  ·  ×1.5 damage');
      this._didCall = true;
    }
  }
  ...
  _updateNumbers(dt, cam, w, h) {
    for (let i = this.numbers.length - 1; i >= 0; i--) {
      ...
      n.node.style.fontSize = `${n.size.toFixed(1)}px`;
