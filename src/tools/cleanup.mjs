...
const liveWorktrees = new Set(
  rows.filter((r) => /node .*tools\/(shoot|perf|gameplay|attrib|sheet)\.mjs/.test(r.args))
    .map((r) => worktreeOf(r.args))
);
// a server whose own child node process is alive is mid-serve, not abandoned
const hasLiveChild = (pid) => rows.some((r) => r.ppid === pid && /vite/.test(r.args) && alive.has(r.pid));

/** Worktree directory gone from disk => the agent that owned it is finished. */
const worktreeGone = (tag) => tag !== 'main'
...
for (const r of candidates) {
  ...
  if (busy) continue;
  r.certain = worktreeGone(tag) || hours >= 3;
  r.tag = tag;
