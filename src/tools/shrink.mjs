...
const val = (n, d) => (argv.indexOf(n) === -1 ? d : argv[argv.indexOf(n) + 1]);
...
async function pickEncoder() {
  ...
  const { CHROMIUM_ARGS } = await import('./chromium.mjs');
  const browser = await chromium.launch({ args: CHROMIUM_ARGS });
  const page = await browser.newPage();
  const encode = async (src, out) => {
    const data = `data:image/png;base64,${(await readFile(src)).toString('base64')}`;
    const b64 = await page.evaluate(async ([uri, q]) => {
